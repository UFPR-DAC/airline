...
export default function Tabela({ colunas, dados, buttons }: Props) {
	...
	const getColumnValue = (item: any, value: string | ((item: any) => any)) => {
		if (typeof value === 'function') return value(item)
		return value.split('.').reduce((acc, key) => acc && acc[key], item)
	}

	const filtrarDados = () =>
		dados.filter((item) =>
			colunas.some((col) => String(getColumnValue(item, col.value)).toLowerCase().includes(filter.toLowerCase()))
		)

	const ordenar = (dados: any[]) => {
		if (!sortedColumn || typeof sortedColumn !== 'string') return dados
	...
	const sortByColumn = (campo: string | ((item: any) => any)) => {
					...
					<table className="w-full whitespace-nowrap text-gray-900 ">
						<thead>
							<tr className="h-16 border border-gray-100 rounded">
								{colunas.map((coluna) => {
									const isSortable = typeof coluna.value === 'string'
									const sortActive = sortedColumn === coluna.value

									return (
										<th
											key={typeof coluna.value === 'string' ? coluna.value : coluna.label}
											className="cursor-pointer max-w-48"
											onClick={isSortable ? () => sortByColumn(coluna.value) : undefined}
										>
											{coluna.label}
											{isSortable && (
												<>
													<FontAwesomeIcon
														icon={faArrowDown}
														className={`ml-2 opacity-45 ${sortActive && sortDirection === 'asc' ? 'text-indigo-600' : ''}`}
													/>
													<FontAwesomeIcon
														icon={faArrowUp}
														className={`ml-1 opacity-45 ${sortActive && sortDirection === 'desc' ? 'text-indigo-600' : ''}`}
													/>
												</>
											)}
										</th>
							...
							{dadosPaginados.length > 0 ? (
								dadosPaginados.map((item, idx) => (
									<tr key={idx} className="h-16 border border-gray-100 rounded text-center">
										{colunas.map((coluna) => (
											<td key={typeof coluna.value === 'string' ? coluna.value : coluna.label}>
												{getColumnValue(item, coluna.value)}
											</td>
										))}
										{buttons.length > 0 && (
											<td>
								...
								<tr>
									<td
										colSpan={colunas.length + (buttons.length > 0 ? 1 : 0)}
